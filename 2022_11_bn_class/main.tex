...
\begin{frame}
	\frametitle{Statification Based Tests: Chi-square Example}
	\begin{block}{}
	\begin{columns}
...
\begin{frame}
	\frametitle{Variable Importance Tests: SCCI Example}
\end{frame}
...
\begin{frame}
	\frametitle{Residualization Based Tests: linear partial-correlation Example}
	\begin{itemize}
...
\section{Unified test for all data types}
\begin{frame}
...
\begin{frame}
	...
	\begin{block}{Ordinal Variable}
			...
			If True value is $ Y_2$, $ R_{Y_2} = \hat{p}(Y < Y_2) - \hat{p}(Y > Y_2) = 0.3 $ \newline
			If True value is $ Y_3$, $ R_{Y_3} = \hat{p}(Y < Y_3) - \hat{p}(Y > Y_3) = 0.9 $
	\end{block}
	\begin{block}{Binary Variable}
			$\hat{p}(Y) = \begin{array}{ll} Y_0 & Y_1 \\ 0.3 & 0.7 \end{array} $ \newline
			If True value is $ Y_0 $, $ R_{Y_0} = Y_0 - \hat{p}(Y=1) = -0.7 $ \newline
			If True value is $ Y_1 $, $ R_{Y_1} = Y_1 - \hat{p}(Y=1) = 0.3 $
	\end{block}
\end{frame}
...
\section{Empirical Results on test performance}
\begin{frame}
