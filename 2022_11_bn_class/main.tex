...
\begin{frame}
	...
	\begin{itemize}
		\item Can utilize any statistical model for which a reasonable goodness
			of fit exist.
		\item Inherently asymmetrical. As the value of $ S(X | Z) - S(X|Y,Z) $ can be different from $ S(Y|Z) - S(Y|X, Z) $,
			the result of $ X \ci Y | Z $ can be different from $ Y \ci X | Z $.
	\end{itemize}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item Since under the null $ g(\bm{\theta}) = 0 $, $\sqrt{n}g(\hat{\bm{\theta}}) \rightarrow \mathcal{N}(0, \sigma^2) \implies Q_1 \to \chi^2(1) $.
	\end{itemize}
