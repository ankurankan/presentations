...
\begin{frame}{Effect Sizes: Marginal Independence Tests}
	...
	\textbf{Discrete Data:} Cram\'er's V
		...
		$$ V_{X, Y} = \sqrt{\frac{\chi^2 / n }{\min(k-1, r-1)}} $$
...
\begin{frame}{Effect Sizes: Conditional Independence Tests}
	...
	\textbf{Discrete Data:} Cram\'er's V
		\begin{itemize}
			\item Stratify data by conditional variable $ \mathcal{D} = \{ (X, Y, \bm{Z}_1), (X, Y, \bm{Z}_2), \cdots, (X, Y, \bm{Z}_k) \} $.
...
\begin{frame}{Effect Sizes: Mixed Data}
	...
	\todo[inline]{Define canonical correlations}
	\textbf{Canonical Correlations:} 
	\begin{itemize}
		\item Generalization of correlation coefficients to set of variables.
		\item Finds linear combination of variables, such that the correlation between them is maximized.
		\item There are multiple effect sizes based on canonical correlations. We use:
...
\begin{frame}{Effect Sizes: Mixed Data}
	...
	\begin{itemize}
		\item If either $ X $ or $ Y $ are discrete, dummy encode them.
			\todo[inline]{Show an example of dummy encoding}
		\item Compute $ Q $.
		\item If both $ X $ and $ Y $ are discrete, this is equivalent to Cram\'er's V.
		\item If both $ X $ and $ Y $ are continuous, this is equivalent to correlation coefficient.
	\end{itemize}
\end{frame}
...
\begin{frame}{Effect Sizes: Mixed Data}
	...
	\begin{itemize}
		\item If either $ X $ or $ Y $ is discrete dummy encode them.
		\item Train two estimators $ f_x: X \sim \bm{Z} $ and $ f_y: Y \sim \bm{Z} $.
