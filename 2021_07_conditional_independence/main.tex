...
\begin{frame}
	...
	\begin{itemize}
		\item CI implies that no direct causal link exists between the variables.
			Example: If $ \text{\emph{Pollution}} \ci \text{\emph{XRay}} | \text{\emph{Cancer}} $,
			there can't be an edge between Pollution and XRay.

		\item Constraint-Based algorithms like PC and FCI use these tests to
			determine model skeletons from data.
	\end{itemize}
\end{frame}
