...
\begin{frame}
	...
	\begin{itemize}
		...
		\item Especially when the cardinality or number of conditional
			variables is high.
		\item For continuous variable case, no test can exist which is
			calibrated and has power over all distributions where
			CI is True. \footnotemark
...
\begin{frame}
	...
	We propose a residualization based approach which requires:
	\vspace{1em}
	\begin{enumerate}
		\setlength\itemsep{1em}
		\item \textbf{Estimator:} Asymptotic results require the
			...
			Model (GLM)).
			\begin{itemize}
				\item We show empirical results using GLM and
					Random Forest.
			\end{itemize}
...
\begin{frame}
	...
	\begin{column}{0.2\textwidth}
		...
		\tikzstyle{every node}=[align=center, inner sep=1pt]
			...
			\node (test) at (0, -2.4) {Test implied and \\ random CIs in data};
			\draw [->] (dag) -- (data);
			\draw [->] (data) -- (test);	
		\end{tikzpicture}
	\end{column}
	...
	\end{column}
	\end{columns}
	\center{Precision and recall  of testing CIs in binary datasets}
\end{frame}
...
\begin{frame}
	...
	\end{column}
	\end{columns}
	\center{F1-score of the learned model skeletons.}
\end{frame}
...
\begin{frame}
	...
	\center{Structure learning on (a) ``alarm'', and (b)
		``insurance'' datasets.  F1-score of the learned model skeletons.}
\end{frame}
...
\begin{frame}
	...
	\begin{column}{0.65 \textwidth}
		...
		\begin{subfigure}{\linewidth}
			\centering
			\includegraphics[scale=0.75]{imgs/sl-adult-rf.pdf}
		\end{subfigure}
		\end{figure}
	\end{column}
	...
	\end{column}
	\end{columns}
	\center{Structure learning on US census income dataset. (top-left)
		Learnt skeleton using chi-squared test, (bottom-left) Learnt skeleton
		using RFT. (right) F1-score when comparing $d$-connected variable pairs
