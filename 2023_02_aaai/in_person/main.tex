...
\begin{frame}
	...
	\begin{figure}
		...
		\end{tikzpicture}
		\caption*{Learned structure for US census income dataset using chi-squared test}
	\end{figure}
	\begin{itemize}
		\item It is much harder to test for CI than for non-conditional
			independence.
		\item This is especially true when the cardinality or the number of 
			conditional variables is high.
		\item For continuous variables, a test which is calibrated and
			has power over all distributions where CI is True can
			not exist. \footnotemark
...
\begin{frame}
	...
	\end{itemize}
	\footnotetext[3]{\footnotesize C. Li and B. E. Shepherd. "A new residual for ordinal outcomes." Biometrika, 2012}
\end{frame}
...
\begin{frame}
	...
	\begin{center}
		\begin{itemize}
			...
			\item $ Q_1 $ is the square root of Generalized Covariance Measure (GCM)\footnotemark.
		\end{itemize}
	\end{center}
	\footnotetext[4]{\footnotesize{Shah, Rajen D., and Jonas Peters. "The hardness of conditional independence testing and the generalised covariance measure." The Annals of Statistics, 2020}}
\end{frame}
...
\begin{frame}{Empirical Analysis}
	\begin{itemize}
		...
		\item We compare GLM and RFT to the following tests:
			...
			\end{enumerate}
	\end{itemize}
	\footnotetext[5]{Marx, Alexander, and Jilles Vreeken. "Testing conditional independence on discrete data using stochastic complexity." The 22nd International Conference on Artificial Intelligence and Statistics. PMLR, 2019.}
\end{frame}
% \begin{frame}
			...
			\begin{figure}
				...
				\begin{subfigure}{\linewidth}
					...
					\begin{equation*}
						\begin{split}
							\bm{z_i} &\sim B(1, 0.5) \\
							\bm{x} &\sim B(1, \bm{z_1} / 3) \\
							\bm{y} &\sim B(1, \bm{z_1} / 3) \\
						\end{split}
					\end{equation*}
				\end{subfigure}
			\end{figure}
...
\begin{frame}
	...
	\begin{columns}
		\begin{column}{0.2 \textwidth}
			\begin{figure}
				...
				\begin{subfigure}{\linewidth}
					\begin{equation*}
						\begin{split}
							p(y) &= S( \sum_{x_i \in \textit{Pa}_y} \bm{\beta} x_i) \\ 
							S(x) &= \frac{1}{1 + e^{-x}}
						\end{split}
					\end{equation*}
				\end{subfigure}
		...
		\end{column}
	\end{columns}
	\center{Accuracy of classifying simulated binary datasets (sample size: $1000$)
	...
	\begin{columns}
		\begin{column}{0.2\textwidth}
			\begin{figure}
				...
				\begin{equation*}
					\begin{split}
						\bm{z_i} &\sim \textit{B}(k-1, 0.5) \\
						\bm{x} &\sim \textit{B}(k-1, \bm{z_1}/k) \\
						\bm{y} &\sim \textit{B}(k-1, \bm{z_1}/k) \\
					\end{split}
				\end{equation*}
				\end{subfigure}
			\end{figure}
		...
		\end{column}
	\end{columns}
	\center{Accuracy of classifying simulated ordinal data (8 levels per
		variable) as conditionally dependent or independent.}	
\end{frame}
...
\begin{frame}
	\frametitle{Empirical Analysis: Model testing}
	\begin{columns}
	\begin{column}{0.2\textwidth}
		\begin{tikzpicture}
		\tikzstyle{every node}=[align=center, inner sep=1pt]
			\node (dag) at (0, 0) {Random DAG \\ ($20$ variables)};
			\node (data) at (0, -1.2) {Simulate binary data \\ (n=$1000$)};
			\node (test) at (0, -2.6) {Test implied and \\ random CI statements \\ in data};
			\draw [->] (dag) -- (data);
			\draw [->] (data) -- (test);	
		\end{tikzpicture}
	\end{column}
...
\begin{frame}
	...
	\begin{column}{0.2\textwidth}
		...
		\tikzstyle{every node}=[align=center, inner sep=1pt]
			\node (dag) at (0, 0) {Random DAG \\ ($20$ variables)};
			\node (data) at (0, -1.2) {Simulate binary data \\ (n=$1000$)};
			\node (sl) at (0, -2.4) {Learn model \\ skeleton};
			\node (test) at (0, -3.4) {Compare skeletons};
			\draw [->] (dag) -- (data);
			\draw [->] (data) -- (sl);
			\draw [->] (sl) -- (test);
		\end{tikzpicture}
...
\begin{frame}
	...
	\end{figure}
	\center{Structure learning on (a) ``alarm'', and (b)
		``insurance'' datasets.}
\end{frame}
...
\begin{frame}
	...
	\begin{column}{0.65 \textwidth}
		...
		\begin{subfigure}{\linewidth}
			\centering
			\includegraphics[scale=0.6]{imgs/sl-adult-rf.pdf}
		\end{subfigure}
		\end{figure}
	\end{column}
	...
	\end{column}
	\end{columns}
	\center{US census income dataset. 
		Learnt skeleton using (top-left) chi-squared test, (bottom-left) 
		RFT. (right) F1-score comparing $d$-connected variable pairs
		from the CPDAG to correlated variable pairs in the dataset.}
\end{frame}
