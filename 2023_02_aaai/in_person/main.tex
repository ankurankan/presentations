...
\begin{frame}
	...
	\begin{itemize}
		\item \textbf{Stratification based tests}: Converts CI tests into a
			bunch of non-conditional independence tests by
			splitting the dataset smaller datasets with constant
			values of $ Z $. Runs the test on each of them and
		...
		\item \textbf{Residulaization based tests}: Uses two estimators $
		...
		\begin{column}{0.2 \textwidth}
			\begin{figure}
				...
				\begin{subfigure}{\linewidth}
					\begin{equation*}
						\begin{split}
							p( y_i = 1 ) &= \lambda( \sum_{X} \bm{\beta} x_i) \\ 
...
\begin{frame}
	...
	\end{column}
	\end{columns}
	\center{Precision and recall (shading: mean $\pm$ standard
...
\begin{frame}
	...
	\begin{column}{0.3\textwidth}
		...
		\tikzstyle{every node}=[align=center, inner sep=1pt]
			\node (dag) at (0, 0) {Random DAG \\ ($20$ variables)};
			\node (data) at (0, -1.2) {Simulate binary data \\ (n=$1000$)};
			\node (sl) at (0, -2.4) {Learn model structure \\ using PC};
			\node (test) at (0, -3.0) {Compare skeletons};
			\draw [->] (dag) -- (data);
			\draw [->] (data) -- (sl);
			\draw [->] (sl) -- (test);
		\end{tikzpicture}
	...
	\end{column}
	\end{columns}
	\center{F1-score (shading: mean $\pm$ standard error) of the learned model
		skeletons.}
\end{frame}
...
\begin{frame}
	...
	\end{column}
	\end{columns}
	\center{Structure learning on US census income dataset. (a)
